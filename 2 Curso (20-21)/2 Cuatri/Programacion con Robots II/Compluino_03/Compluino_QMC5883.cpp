/*!
 * @file COMPLUINO_QMC5883.cpp
 * @brief Compatible with QMC5883 and QMC5883
 * @n 3-Axis Digital Compass IC
 *
 * @copyright GNU Lesser General Public License
 *
 * @author Complubot, basado en la librería DFROBOT_QMC5883 [DFRobot](http://www.dfrobot.com), 2017
 * @version Compluino 1.0 
 */
...
Vector COMPLUINO_QMC5883::readRaw(void)
{
  int range = 10;
  float Xsum = 0.0;
  float Ysum = 0.0;
...
void COMPLUINO_QMC5883::calibrate()
{
...
void COMPLUINO_QMC5883::initMinMax()
{
...
Vector COMPLUINO_QMC5883::readNormalize(void)
{
  int range = 10;
  float Xsum = 0.0;
  float Ysum = 0.0;
...
void COMPLUINO_QMC5883::setRange(QMC5883_range_t range)
{
  if(isHMC_){
    switch(range){
...
QMC5883_range_t COMPLUINO_QMC5883::getRange(void)
{
  if(isHMC_){
...
void COMPLUINO_QMC5883::setMeasurementMode(QMC5883_mode_t mode)
{
  uint8_t value;
  if(isHMC_){
...
QMC5883_mode_t COMPLUINO_QMC5883::getMeasurementMode(void)
{
  uint8_t value=0;
  if(isHMC_){
    value = readRegister8(HMC5883L_REG_MODE);
...
void COMPLUINO_QMC5883::setDataRate(QMC5883_dataRate_t dataRate)
{
  uint8_t value;
  if(isHMC_){
...
QMC5883_dataRate_t COMPLUINO_QMC5883::getDataRate(void)
{
  uint8_t value=0;
  if(isHMC_){
    value = readRegister8(HMC5883L_REG_CONFIG_A);
...
void COMPLUINO_QMC5883::setSamples(QMC5883_samples_t samples)
{
  uint8_t value;
  if(isHMC_){
    value = readRegister8(HMC5883L_REG_CONFIG_A);
...
QMC5883_samples_t COMPLUINO_QMC5883::getSamples(void)
{
  uint8_t value=0;
  if(isHMC_){
    value = readRegister8(HMC5883L_REG_CONFIG_A);
...
void COMPLUINO_QMC5883::writeRegister8(uint8_t reg, uint8_t value)
{
  if(isHMC_){
    Wire.beginTransmission(HMC5883L_ADDRESS);
    #if ARDUINO >= 100
...
uint8_t COMPLUINO_QMC5883::fastRegister8(uint8_t reg)
{
  uint8_t value=0;
  if(isHMC_){
    Wire.beginTransmission(HMC5883L_ADDRESS);
...
uint8_t COMPLUINO_QMC5883::readRegister8(uint8_t reg)
{
  uint8_t value=0;
  if(isHMC_){
    Wire.beginTransmission(HMC5883L_ADDRESS);
...
int16_t COMPLUINO_QMC5883::readRegister16(uint8_t reg)
{
  int16_t value=0;
  if(isHMC_){
    Wire.beginTransmission(HMC5883L_ADDRESS);
...
int COMPLUINO_QMC5883::getICType(void)
{
  if(isHMC_){
    return IC_HMC5883L;
  }else if(isQMC_){
