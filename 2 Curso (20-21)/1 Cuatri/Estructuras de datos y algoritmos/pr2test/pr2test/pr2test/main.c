...
FILE *pfich3;
char *ptrtoken;
tEmpleado listaEmpleados[100];
tRango listaRango[100];
long sueldos[100];
int finalizador;
int opcion;
int contadorNE = 0;
...
void imprimirDireccion(tEmpleado *puntero){
...
void anadirEmpleado(tEmpleado *e){
    int i = 0;
    while(!strcmp(listaEmpleados[i].Nombre , "")){
        i++;
    }
    listaEmpleados[i] = *e;
}
...
int LeerFicheroE (char NomFichero[], tEmpleado *e)
{
    ...
    if(pfich1 == NULL){
        ...
        tEmpleado *auxiliar = (tEmpleado *) malloc(sizeof(tEmpleado));
        ...
        while(fgets(linea, 100,  pfich1)){
            ...
            if(ptrtoken != NULL){
                strcpy(auxiliar->Nombre, ptrtoken);
            }
            if((ptrtoken = strtok(NULL, SEPARADOR)) != NULL){
                strcpy(auxiliar->Apellido1 , ptrtoken);
            }
            if((ptrtoken = strtok(NULL, SEPARADOR)) != NULL){
                strcpy(auxiliar->Apellido2 , ptrtoken);
            }
            if((ptrtoken = strtok(NULL, SEPARADOR)) != NULL){
                auxiliar-> Sueldo = strtol(ptrtoken,NULL,10);
            }
...
int EscribirFichero (char NomFichero[], tEmpleado *e)
{
    ...
    if(pfich3 == NULL){
        ...
        CalcularNuevosSueldos(listaEmpleados, listaRango, NE, NR);
        for(int i = 0 ; i < NE ; i++){
...
// --------------------------------------------------------------
    ...
    // punteroEmpleado = (tEmpleado *) malloc(sizeof(tEmpleado));
    ...
    while(finalizador == 0){
        ...
        switch (opcion) {
            case OPCION_UNO:
                ...
                LeerFicheroE(fichero1 , listaEmpleados);
            ...
            case OPCION_TRES:
                ...
                CalcularNuevosSueldos(listaEmpleados, listaRango , NE, NR);
                EscribirFichero(fichero3, listaEmpleados);
                break;
